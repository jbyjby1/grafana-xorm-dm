...
func (db *dm) IndexCheckSql(tableName, idxName string) (string, []interface{}) {
	...
	sql := "SELECT INDEX_NAME FROM SYS.USER_INDEXED WHERE TABLE_OWNER = ? AND TABLE_NAME = ? AND INDEX_NAME = ?"
	return sql, args
}
...
func (db *dm) TableCheckSql(tableName string) (string, []interface{}) {
	...
	sql := "SELECT TABLE_NAME FROM SYS.ALL_TABLES WHERE OWNER = ? AND TABLE_NAME = ?"
	return sql, args
}
...
func (db *dm) GetColumns(tableName string) ([]string, map[string]*core.Column, error) {
	...
	s := `
	SELECT SYS.ALL_TAB_COLUMNS.COLUMN_NAME, NULLABLE, DATA_DEFAULT, DATA_TYPE, CONSTRAINT_TYPE, AUTOI.AUTOI_PRIMARY_KEY AS AUTOI, USER_COL_COMMENTS.COMMENTS AS COLUMN_COMMENT 
FROM SYS.ALL_TAB_COLUMNS
LEFT JOIN SYS.ALL_CONS_COLUMNS ON 
SYS.ALL_CONS_COLUMNS.OWNER = SYS.ALL_TAB_COLUMNS.OWNER AND SYS.ALL_CONS_COLUMNS.TABLE_NAME = SYS.ALL_TAB_COLUMNS.TABLE_NAME 
LEFT JOIN SYS.ALL_CONSTRAINTS ON
SYS.ALL_CONS_COLUMNS.CONSTRAINT_NAME = SYS.ALL_CONSTRAINTS.CONSTRAINT_NAME
LEFT JOIN (
SELECT
    OBJ.OWNER AS AUTOI_OBJ_OWNER,
    TAB.NAME AS AUTOI_TABLE_NAME,
    COL.NAME AS AUTOI_COLUMN_NAME,
    '1' AS AUTOI_PRIMARY_KEY
FROM 
    SYSOBJECTS TAB,
    SYSCOLUMNS COL,
    DBA_OBJECTS OBJ 
WHERE 
    COL.ID = TAB.ID 
    AND OBJ.OBJECT_ID = TAB.ID 
    AND TAB.TYPE$ = 'SCHOBJ' 
    AND TAB.SUBTYPE$ IN ('UTAB') 
    AND COL.INFO2 & 0x01 = 1
) AUTOI ON SYS.ALL_TAB_COLUMNS.OWNER = AUTOI.AUTOI_OBJ_OWNER 
AND SYS.ALL_TAB_COLUMNS.TABLE_NAME = AUTOI.AUTOI_TABLE_NAME AND SYS.ALL_TAB_COLUMNS.COLUMN_NAME = AUTOI.AUTOI_COLUMN_NAME
LEFT JOIN USER_COL_COMMENTS ON SYS.ALL_TAB_COLUMNS.OWNER = USER_COL_COMMENTS.OWNER 
AND SYS.ALL_TAB_COLUMNS.TABLE_NAME = USER_COL_COMMENTS.TABLE_NAME AND SYS.ALL_TAB_COLUMNS.COLUMN_NAME = USER_COL_COMMENTS.COLUMN_NAME
WHERE SYS.ALL_TAB_COLUMNS.OWNER = ? AND SYS.ALL_TAB_COLUMNS.TABLE_NAME = ?`
	...
	for rows.Next() {
		...
		var columnName, isNullable, colType, colKey, autoi, comment string
		var colDefault *string
		err = rows.Scan(&columnName, &isNullable, &colDefault, &colType, &colKey, &autoi, &comment)
		if err != nil {
			return nil, nil, err
		}
		...
		if !col.DefaultIsEmpty {
			...
			} else if col.SQLType.IsTime() && col.Default != "SYSDATE" {
				col.Default = "'" + col.Default + "'"
			}
		}
		cols[col.Name] = col
...
func (db *dm) GetTables() ([]*core.Table, error) {
	...
	// return tables, nil
	args := []interface{}{}
	s := "SELECT TABLE_NAME FROM SYS.ALL_TABLES"
	if len(db.Schema) != 0 {
...
func (db *dm) GetIndexes(tableName string) (map[string]*core.Index, error) {
    ...
    CASE WHEN a.UNIQUENESS = 'UNIQUE' THEN 0 ELSE 1 END AS NON_UNIQUE,
...
JOIN 
    USER_IND_COLUMNS b 
ON 
    a.TABLE_NAME = b.TABLE_NAME AND a.INDEX_NAME = b.INDEX_NAME 
WHERE 
	a.TABLE_OWNER = ? 
    AND a.TABLE_NAME = ? 
ORDER BY 
    a.INDEX_NAME, b.COLUMN_POSITION`
...
func (p *dmDriver) Parse(driverName, dataSourceName string) (*core.Uri, error) {
